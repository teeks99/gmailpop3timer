...
/* 
    ...
    Copyright (C) 2022 Thomas Kent. All Rights Reserved. 
    ...
    var o = {
        ...
        core: {
            ...
            init: function() {
                ...
                o.core.el.settingsLink.style.display = 'none'; /* Google+ with black bar added on 2/11/2012 */
                if ((document.getElementById('gbx3') || document.getElementById('gbx4')) && !document.getElementById('gbgs5')) {
                    ...
                    o.core.el.settingsLink.style.textDecoration = 'none'; /* The New Gmail look */
                    ...
                    tempNode.appendChild(o.core.el.body);
                    o.core.el.header.appendChild(tempNode);
                    o.core.updateDisplay(); /* Banner top added on 11/30/2011 */
                    ...
                    o.core.el.body.style.zIndex = '99999'; /* TODO: temporary */
                    o.core.el.body.style.whiteSpace = 'no-wrap'; /* Google+ bar */
                    if (document.getElementById('gbi4t')) {
                        ...
                        o.core.el.body.style.top = '24px'; /* Not Google+ */
                        ...
                        o.core.setting.template = 3; /*} else {console.log('Gmail POP3 Checker => Template: New google banner added 9/20/2013');var wrapper = document.getElementById(':hq');if (wrapper) {o.core.el.header = document.createElement('div');wrapper.parentNode.insertBefore(o.core.el.header, wrapper);o.core.el.body.style.position = 'static';o.core.setting.template = 6;}*/
                    }
                    o.core.el.body.style.padding = '0';
                    o.core.el.body.style.textAlign = 'right';
                    o.core.el.header.appendChild(o.core.el.body);
                    o.core.el.donationLink.style.marginLeft = '4px'; /* new banner top (Google+) */
                    ...
                    t1.style.display = 'inline-block'; /* t1.style.borderLeft = '1px solid #EEE'; */
                    ...
                    o.core.el.body = t1;
                    o.core.el.header.appendChild(o.core.el.body);
                    o.core.el.donationLink.style.marginLeft = '4px'; /* TODO: temporary */
                    o.core.el.body.style.whiteSpace = 'no-wrap'; /* old banner top */
                } else {
                    console.log('Gmail POP3 Checker => Template: Old banner top');
                    o.core.setting.template = 1;
                    o.core.el.header = document.body.getElementsByTagName('nobr')[1]; /* try to find the Settings link based on the href */
                    var elA = o.core.el.header.getElementsByTagName('a');
                    for (var i = 0; i < elA.length; i++) {
                        if (elA[i].getAttribute('href') && elA[i].getAttribute('href').match(/#settings/gi)) { /* yay! we've found a match for the settings link */
                            o.core.el.sibling = elA[i];
                            o.core.el.sibling.parentNode.insertBefore(o.core.el.body, o.core.el.sibling.nextSibling);
                        }
                    }
                    if (!o.core.el.sibling) { /* not found, eh? just slap it at the end then */
                    ...
                    } /* are labs installed? */ /*if (o.core.el.header.innerHTML.match(/labs_bar_icon.png/ig)) {o.core.el.body.appendChild(document.createTextNode(' | '));}*/
                ...
                if (o.version != 'Static Versioned') {
                    ...
                    o.core.el.body.appendChild(o.core.el.pop3); /* are labs not installed? */
            ...
            refresh: function() {
                ...
                } else { /* The refresh link was never found or is not available. */ }
            },
            pop: function() {
                if (!o.core.checking) {
                    ...
                    o.core.el.pop3.appendChild(document.createTextNode('Fetching...')); /* we'll throw a timeout. in the event the POP3s fail we'll trigger a restart. this happens particulary when the computer sleeps, hybernates, or looses internet connectivity. */
                    window.setTimeout(function() {
                        ...
                        if (!isCompleted) {
                            ...
                            window.setTimeout(function() {
                                var d = new Date(); /* set the timer for 3 minutes instead of the designated interval */
                    ...
                    for (var i = 0; i < o.core.request.url.length; i++) {
                        var responseFunc = function(r) {
                            var isCompleted = true; /* this is just updating the first one not marked as completed. we should really be marking the one returned, but this type of logic doesn't matter for our check */
                            for (var i = 0; i < o.core.request.id.length; i++) {
                            ...
                            } else { /* all of the pop accounts have not completed yet */ }
                        }; /* Firefox, or Chrome before version 27.X */
                        if (typeof GM_xmlhttpRequest == 'function') {
                            o.request(o.domain + o.core.request.url[i], responseFunc, 'POST'); /* Chrome after and including version 27.X */
                        } else { /* we need to keep these xhr things private to their own block. do I love this solution? no */
                            myFunc = function() {
                                var xhr = new XMLHttpRequest();
            ...
            timer: function() { /* is it time to POP3 yet? */
                var d = new Date();
            ...
            account: function() {
                ...
                var responseFunc = function(r) {
                    ...
                    for (var i = 0; i < temp.length; i++) {
                        if (temp[i].innerHTML && temp[i].innerHTML.match(/var GLOBALS/gi)) {
                            var count = 0;
                            temp = temp[i].innerHTML.substr(temp[i].innerHTML.indexOf('GLOBALS=') + 1, temp[i].innerHTML.length); /* grab everything until GLOBALS[X] gets set (which should be everything within GLOBALS) */
                            temp = temp.substr(0, temp.indexOf('GLOBALS[')); /* trim */
                            temp = temp.replace(/^\s+|\s+$/g, ''); /* strip off the last character which should be a ';' */
                            temp = temp.substr(0, temp.length - 1); /* evaluate the GLOBALS into our own object */
                            o.core.globals = eval(temp); /* ik */
                            o.core.ik = o.core.globals[9]; /* account/email */
                            o.core.account = o.core.globals[10]; /* get the GMAIL_AT (aka: AT) from the cookies. It's basically the current session ID which is required to make POP3 requests. */
                            temp = document.cookie;
                            temp = temp.substr(temp.indexOf('GMAIL_AT=') + 9, temp.length);
                            o.core.at = temp.substr(0, temp.indexOf(';')); /* Template before April 25, 2018 */
                            if (o.core.globals[17] !== null) {
                                console.log('Gmail POP3 Checker => AMA: v1'); /* request.account account/email/ama */
                                ...
                                for (var j = 0; j < o.core.globals[17][o.core.amaposition][3].length; j++) {
                                    ...
                                    o.core.request.url[j] = location.pathname + '?ui=2&ik=' + o.core.ik + '&at=' + o.core.at + '&view=up&act=cma_' + o.core.request.id[j] + '&pcd=1&mb=0&rt=h'; /* https://mail.google.com/mail/u/0/?ui=2&ik=64e9f3348b&rid=f6a..&at=AF6bupN8dlBRtQFohddzVDUO9IL6BqjbUg&view=up&act=cma_5&_reqid=1552177&pcd=1&mb=0&rt=h&zx=9gvi70xjxb4b */ /* o.core.request.url[j] = location.pathname + '?ui=1&ik=&view=up&act=cma_' + o.core.request.id[j] + '&at=' + o.core.at; */
                                }
                                loadedFunc(); /* Template after April 25, 2018 which moved the ama to the end of the page outside of the GLOBALS=[] variable */
                            } else { /* \u0022 is an escaped  */
                                ...
                                if (str.length) {
                                    ...
                                    console.log('Gmail POP3 Checker => AMA: v3'); /* Last hope for the ama (check the refresh mail page) */
                                    var amaFunc;
                                    (function() {
                                        amaFunc = function(r) {
                                            if (r.status == '200') {
                                    ...
                                    if (typeof GM_xmlhttpRequest == 'function') {
                                        o.request(o.domain + location.pathname + '?ui=2&ik=' + o.core.ik + '&at=' + o.core.at + '&view=up&act=par&rt=h', amaFunc); /* Chrome after and including version 27.X */
                                    } else {
                                        var xhr = new XMLHttpRequest();
                                        xhr.onreadystatechange = function(e) {
                                            if (xhr.readyState == this.DONE) {
                ...
                if (typeof GM_xmlhttpRequest == 'function') { /* snag your google identification hash (ik) */
                    o.request(o.domain + location.pathname + '?ui=2', responseFunc); /* Chrome after and including version 27.X */
                } else {
                    var xhr = new XMLHttpRequest();
                    xhr.onreadystatechange = function(e) {
                        if (xhr.readyState == this.DONE) {
